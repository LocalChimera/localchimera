...
export default function WikiPage({ onBack }) {
  const privy = isNative ? { authenticated: true, login: () => {}, logout: () => {}, user: null } : usePrivy();
  ...
  const startNode = async () => {
    if (!isNative && (!authenticated || !user?.wallet?.address)) {
    ...
    try {
      const evmAddr = isNative ? null : user.wallet.address;
      const res = await fetch(`${API_BASE}/start`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ evmAddress: evmAddr })
      });
    ...
    <div style={{ ...s.layout, ...(isMobile ? { flexDirection: 'column', height: 'auto', overflow: 'auto' } : {}) }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          {isNative ? (
            <div style={{ fontSize: 10, color: '#86efac' }}>On-device node</div>
          ) : authenticated ? (
        ...
        <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(255,255,255,0.06)', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            <button
              ...
              onClick={startNode}
              disabled={nodeRunning || (!isNative && !authenticated)}
              title={!isNative && !authenticated ? 'Log in to start mining' : ''}
            >
              ▶ Start
            </button>
            <button
              ...
              onClick={stopNode}
              disabled={!nodeRunning || (!isNative && !authenticated)}
              title={!isNative && !authenticated ? 'Log in to stop mining' : ''}
            >
              ⏹ Stop
            </button>
          </div>
